...
class SUNRGBDData(object):
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pc_upright_depth_subsampled = np.load("/remote-home/linzhx/Datasets/SUNREFER/sunrgbd_pc_bbox_votes_30k_v2/{:06}_pc.npz".format(sample_idx))['pc']
            info = dict()
            ...
            if has_label:
                ...
                annotations = {}
                annotations['gt_num'] = len([obj.classname for obj in obj_list if obj.height != 0])
                if annotations['gt_num'] != 0:
                    annotations['name'] = np.array(['object' for obj in obj_list if obj.height != 0])
                    annotations['bbox'] = np.concatenate([obj.box2d.reshape(1, 4) for obj in obj_list if obj.height != 0], axis=0)
                    annotations['location'] = np.concatenate([obj.centroid.reshape(1, 3) for obj in obj_list if obj.height != 0], axis=0)
                    annotations['dimensions'] = 2 * np.array([[obj.length, obj.width, obj.height] for obj in obj_list if obj.height != 0])  # lwh (depth) format
                    annotations['rotation_y'] = np.array([obj.heading_angle for obj in obj_list if obj.height != 0])
                    annotations['index'] = np.arange(len(obj_list), dtype=np.int32)
                    annotations['class'] = np.array([0 for obj in obj_list if obj.height != 0])
                    annotations['gt_boxes_upright_depth'] = np.stack([obj.box3d for obj in obj_list if obj.height != 0], axis=0)  # (K,8)
...
if __name__ == '__main__':
    val_dataset = SUNRGBDData(root_path='.', split='val', use_v1=False)
    infos_train = val_dataset.get_infos(num_workers=4, has_label=True)
    mmcv.dump(infos_train, 'sunrgbd_det_val.pkl', 'pkl')
    print(f'info val file is saved')

    train_dataset = SUNRGBDData(root_path='.', split='train', use_v1=False)
    infos_train = train_dataset.get_infos(num_workers=4, has_label=True)
    mmcv.dump(infos_train, 'sunrgbd_det_train.pkl', 'pkl')
    print(f'info train file is saved')
